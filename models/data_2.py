...
# axs[0].plot(range(len(group_statistics_mean)), group_statistics_mean['var'], marker='o', linestyle='-', linewidth=2)
# axs[0].set_ylabel('Mean')
#
# axs[1].plot(range(len(group_statistics_std)), group_statistics_std['var'], marker='o', linestyle='-', linewidth=2)
# axs[1].set_ylabel('Standard Deviation')
#
# axs[2].plot(range(len(group_statistics_quantile25)), group_statistics_quantile25['var'], marker='o', linestyle='-', linewidth=2)
# axs[2].set_ylabel('25th Percentile')
#
# axs[3].plot(range(len(group_statistics_var)), group_statistics_var['var'], marker='o', linestyle='-', linewidth=2)
# axs[3].set_ylabel('Variance')
#
# axs[4].plot(range(len(group_statistics_skew)), group_statistics_skew['var'], marker='o', linestyle='-', linewidth=2)
# axs[4].set_ylabel('Skewness')

axs[0].plot(range(len(group_statistics_quantile75)), group_statistics_quantile75['var'], marker='o', linestyle='-', linewidth=2)
axs[0].set_ylabel('75th Percentile')

axs[1].plot(df_current.index, rolling_mean['var'], linestyle='-', linewidth=2)
axs[1].set_ylabel(f'{window_size}-MA')

axs[2].plot(df_current.index, rolling_std['var'], linestyle='-', linewidth=2)
axs[2].set_ylabel(f'{window_size}-MSD')

axs[3].plot(df_current.index, rolling_kurtosis['var'], linestyle='-', linewidth=2)
axs[3].set_ylabel(f'{window_size}-Kurtosis')

axs[4].plot(df_current.index, exponential_smoothing['var'], linestyle='-', linewidth=2)
